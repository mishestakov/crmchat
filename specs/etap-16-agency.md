...
| Фаза | Что | Статус |
|---|---|---|
| Бриф | форма (бюджет/период/TOV/ограничения) | ✅ реализовано |
...
| Подтверждение | bulk-send approved-блогерам «вы выбраны» | ✅ реализовано |
| Запуск | pipeline-матрица (§16.5) | ✅ реализовано |
| Отчёт | снятие метрик постов (TDLib) + агрегаты | ✅ метрики; постотчёт-выводы и шеринг клиенту отложены |
...
### 16.4 Подтверждение = разовый bulk-send (не цепочка)
...
### 16.5 Запуск — pipeline-матрица (шаги 5–13 исходного процесса)
...
- ✅ Web: tree «Клиенты→Кампании» + карточка клиента; визард со всеми 6 фазами
  (Бриф / Лонглист / Согласование / Подтверждение / Запуск / Отчёт);
  клиентский `/share/$token` view.
- ✅ Отчёт: `metrics-worker.ts` снимает метрики постов через TDLib по событийной
  модели (`getMessageLinkInfo` → `openChat` → `viewMessages(force_read)` → ждём
  `updateMessageInteractionInfo`, fallback на baseline; троттл 10с/100 в час).
  Снимок поста (текст + минитамбнейл) + views/forwards/reactions на
  `project_items.metrics_*` + `post_snapshot`. Кнопка «снять статистику со всех»
  (`POST .../collect-metrics`), агрегаты (Σохват, Σбюджет, средний CPV).
- ✅ Бриф: даты проведения (`period_start/end`), tov — textarea, KPI убран из UI.
- ✅ Лонглист — массовое добавление: модалка с табами «Несколько» (textarea, по
  URL/@username на строку, по умолчанию) и «Один» (поиск). `POST
  .../placements/bulk` делает find-or-create канала (нет в базе — болванка
  `@username`, реальные данные подтянет ленивый sync) и заводит размещения.
- ✅ Переиспользование со страницы Каналов: клик по столбцу «Канал» на любом
  этапе открывает `ChannelDrawer` — лента постов, авто-синк статистики
  («открыл — актуализировалось», `POST /channels/{id}/sync`), секция админов.
  Контакт админа можно завести прямо в сайдбаре по @username (`POST
  /channels/{id}/admins` принимает `usernames` → find-or-create stub).
- ⏳ Отложено: **способы связи user/group/bot** — сейчас получатель размещения
  только личный контакт-админ (DM), worker пишет лишь в личку не-ботам
  (`resolveAndCheckNotBot`). Групповой админ-чат / бот как способ связи + авто-
  резолв админов канала (`getChatAdministrators`) — отдельный заход (трогает
  outreach-механику). Также: постотчёт-выводы и шеринг отчёта клиенту (шаг 13);
  фоновое авто-обновление просмотров; `placement_files`, `placement_comments`.

### 16.8 Лонглист 2.0 — подготовка контактов + жёсткий гейт (ревизия §16.2)

**Проблема §16.2.** Текущий лонглист построен вокруг чата с админом и
предполагает, что контакт уже есть, а `WaveModal` склеивает «добавить
блогеров» = «сразу запустить рассылку + активировать». На практике менеджер
получает список каналов, где у части контакта нет, и должен сначала
**подготовить** список (найти/завести контакты, оценить каналы), и только
потом запускать аутрич. Этот этап в UI отсутствует.

**Что меняем.** Лонглист становится этапом **подготовки контактов с жёстким
гейтом на запуск**. «Добавить в лонглист» (без отправки) и «Запустить аутрич»
— разные действия.

**Раскладка (3 колонки, правый рельс морфится):**
- Список слева — **плоско по каналам** (каждая строка = канал, своя
  цена/метрики). У каналов одного админа — бейдж-хинт «+N каналов этого
  админа» и общий тред. Группировку-сворачивание НЕ делаем.
- Центр — карточка канала (тот же `ChannelCard`: метрики, описание, лента),
  видна всегда.
- Правый рельс по состоянию строки: **нет контакта** → резолвер (суджест-чип
  из описания · поиск по базе · ручной ввод · «в личку канала» · убрать
  канал); **есть контакт** → чат с админом + поля сделки (готов/цена/прогноз).

**Обогащение на добавлении (была лень — стала автоматизация):**
- Скан канала (`POST /channels/{id}/sync` + лента) запускается **сразу при
  добавлении** в лонглист фоном (не лениво при открытии drawer'а), чтобы
  переключение между каналами было мгновенным с готовыми метриками. In-process
  fire-and-forget с троттлом; на рестарт API подстрахован ленивый sync. Нужен
  active outreach-аккаунт.
- Резолв контакта: (a) матч по базе/`channel_admins` [есть на CSV-импорте],
  (b) DM-путь канала, (c) regex-экстракт `@username`/`t.me` из описания и
  закрепа → **суджест-чип** (менеджер подтверждает в один клик; молча
  `channel_admins` не пишем — ложные срабатывания не засоряют базу).

**DM-путь канала (`has_dm`).** Канал с direct-messages-группой засчитывается
готовым контактом, **только если отправка бесплатна**
(`supergroupFullInfo.outgoing_paid_message_star_count == 0`,
`direct_messages_chat_id != 0`; см. td_api.tl). Если стоимость > 0 → бейдж «в
личку: N⭐/сообщ», для гейта НЕ считается, менеджер пишет руками (ручной
quick-send, авто мимо). Sync уже снимает `outgoing_paid_message_star_count` и
`direct_messages_chat_id` — надо прокинуть в API/UI и в логику готовности.

**Готовность и жёсткий гейт:**
- Статус контакта на строке: есть / суджест / нет. Метрик-чипы (подписчики ·
  ср. охват · ERR · DM-стоимость).
- В шапке прогресс «Контакты X/Y · Z без контакта».
- **Жёсткий гейт**: «Запустить аутрич» активна только при 0 каналов без
  контакта. Единственный выход для пустой строки — «убрать канал». На доливке
  гейт проверяется по новой волне (не только на первом запуске).
- Готовность по факту **пер-админная**: известен админ → все его каналы
  готовы; не готов только канал без админа и без бесплатного DM.

**Авторассылка при N каналов у одного админа — цепочка живёт на админе, не на
плейсменте:**
- Активация в agency-ветке **коллапсит плейсменты по `adminContactId`** → один
  набор `scheduled_messages` на админа (сейчас `buildScheduledRows` идёт по
  item'ам). Один опенер на человека, не N.
- Опенер называет каналы через плейсхолдер **`{{каналы}}`**. Движок шаблонов
  (`substitute-variables.ts`, снапшот на активации) править не надо: при
  группировке синтезируем свойство `каналы = "@a, @b, https://…"`, и
  `{{каналы}}` разворачивается существующим lookup'ом по `lead.properties`.
  Идентификатор канала — естественный: TG → `@username`, YouTube/TikTok/Дзен →
  ссылка (не title: названия в опенере не нужны, @handle/URL однозначнее).
- Снапшот фиксирует список каналов в тексте на момент запуска → канал того же
  админа, долитый позже, в уже ушедший опенер не попадёт (= повторный опенер
  не шлём, новые каналы менеджер упоминает в живом треде).
- Ответ в чате админа разливается на **все** его плейсменты в проекте (сейчас
  reply маппится 1:1 на item). Пер-канальное готов/цена — руками менеджера в
  общем треде (без парсинга).

**MVP-минимум этого подэтапа:** эагерный скан на добавлении + матч-по-базе +
DM-путь (бесплатный) + экстракт-суджест + статус/прогресс/жёсткий гейт + дедуп
опенера по админу + `{{каналы}}` + 3-колоночная раскладка.

**Отложено:** авто-вычисление ср. охвата и ERR (прогноз остаётся ручным;
формула ERR — по reach vs подписчикам — уточнить отдельно); группировка-
сворачивание списка по админу; авто-расписание сбора метрик (§8.4).

### 16.9 Смена контакта-админа + эпик «способы связи» (бот/группа)

**Часть 1 — смена контакта (реализовано, этап 16.8 / п.1).** Получатель канала
можно поменять, не пересоздавая размещение. В рельсе с контактом — «сменить»;
в резолвере — три варианта: контакт из базы · новый @ · **в личку канала
(0⭐)**. Эндпоинт `POST /channels/{id}/set-admin` (`contactId | username | dm`).

- **Scope — глобально по каналу** (согласовано): «кто ведёт канал» — факт о
  канале, не о кампании. `set-admin` заменяет `channel_admins` на выбранного
  (single-recipient модель) и перенаводит ВСЕ размещения канала (across
  projects) через `healPlacementRecipients(override)`; `dm` — снимает персону
  и обнуляет получателя (`clearPlacementRecipients`), готовность держится на
  бесплатном DM. Сама сущность-`contact` при перепривязке НЕ мутируется —
  меняется лишь, какой контакт привязан. Правка протухшего @username — это
  отдельно, через карточку контакта (`/contacts/:id`), и она глобальна по
  природе (один contact на всю базу).
- Перенаводка влияет на будущие отправки/чат, не на уже ушедшие (актуально
  только при смене посреди активной кампании; обычно меняют на лонглисте).
- Per-project-расхождение (разные посредники на один канал в разных кампаниях)
  — реальный, но редкий кейс; в дефолт не берём, при нужде — явный override.

**Часть 2 — эпик «способы связи» (НЕ реализовано, отдельный заход).** Сейчас
способ связи = первый `channel_admins → contact` (человек). Надо
**гетерогенный «способ связи» на канале**: `человек-контакт | бот |
личка-канала | группа` (вместо «первый админ выигрывает»). Часть 1 (явный
выбор человек/DM) — первый шаг этой модели.

- **Реальность Telegram (важно, чтобы не строить иллюзию).** Список админов
  чужого канала TG **не отдаёт** (`getChatAdministrators` — только там, где ты
  сам админ). Значит авто-перечня людей-админов по холодному каналу не будет.
  Реальные источники кандидатов: `linked_chat_id` (группа-обсуждение) и
  `direct_messages_chat_id` (личка-группа канала) — уже снимаем на скане; плюс
  @-упоминания / инвайты из описания и закрепа (бот по рекламе, ссылка в
  группу) — уже есть в суджест-чипах. То есть «импортировать ботов и группы» =
  захватывать связанную группу + DM + парсить упоминания, а не «получить всех
  админов».
- **Авто vs вручную (предложение, требует подтверждения при старте эпика):**
  - **Авто-рассылка**: личка человека + бесплатная личка канала.
  - **Вручную** (менеджер пишет сам; гейт засчитывает как готовое): бот, группа,
    платная личка.
- **Главная цена — worker, не импорт/UI.** Сейчас аутрич умеет писать только в
  личку человеку-не-боту (`resolveAndCheckNotBot`). Бот / группа / личка-канала
  — другие цели отправки (`chat_id` вместо `user`, боту нужен `/start`, платный
  `paid_message_star_count` в `messageSendOptions`). Это и есть отложенный §16.7
  («способы связи user/group/bot»).
- **Модельная зацепка:** группа и личка-канала — это НЕ `contact` (там нет
  человека). Значит способ связи — отдельная сущность с типом + ссылкой
  (на contact, либо на chat_id канала/группы), а не просто ещё одна строка
  `channel_admins`. Бот — это `contact` с `userTypeBot`.
- UI: в резолвере список кандидатов-способов (DM/группа/бот/упоминания) →
  менеджер выбирает; авто-способы идут в цепочку, ручные — помечаются как
  «писать вручную».

**Решения, нужные до старта эпика:** финальное авто-vs-вручную по каждому типу;
форма хранения способа связи (тип+ref); поведение worker'а для group/bot/
channel-DM (включая платный send-option).
