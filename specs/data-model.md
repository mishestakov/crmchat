...
| `telegram_username` | text, nullable | TG-username, если известен |
...
| `id` | uuid (pk) |
| `workspace_id` | uuid → `workspaces.id` ON DELETE CASCADE |
| `account_id` | uuid → `telegram_accounts.id` ON DELETE CASCADE |
...
`telegram_accounts.session` хранит MTProto auth-keys — компрометация даёт полный доступ к TG-аккаунту. Применяем колоночное шифрование через pgcrypto:
