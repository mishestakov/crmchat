...
export type LeadHealthInput = {
  ...
  // Время последнего сообщения в треде В ЛЮБУЮ сторону (/leads считает его как
  // greatest(contacts.last_message_at [входящее], tg_chats [наши исходящие,
  // включая ручные])). От него и считаем застой — поэтому ручной ответ менеджера
  // сбрасывает «затихло». null — переписки ещё не было (или контакта нет).
