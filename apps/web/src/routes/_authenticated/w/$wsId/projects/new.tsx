...
// Форма создания проекта: папка + имя + шаблон стадий. Стадии копируются
// из шаблона в project.stages при create. CSV-импорт лидов — отдельный
// flow на странице проекта (12.5).
//
// UX-эвристика: дефолтный шаблон в селекте — последний использованный в
// этой папке (хранится в localStorage по trackId). Это закрывает кейс
// «у клиента 99% одинаковый канбан» без явного «папка имеет шаблон».
...
function NewProjectPage() {
  ...
  // Когда выбрали папку — подставить последний использованный шаблон в
  // этой папке (если он ещё существует в текущих templates).
  useEffect(() => {
    if (!trackId) return;
  ...
  const create = useMutation({
    ...
    onSuccess: (project) => {
      // Запоминаем выбор шаблона в localStorage — следующий проект в этой
      // папке откроется с этим же дефолтом.
      if (templateId) {
