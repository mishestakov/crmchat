...
export const Route = createFileRoute(
  "/_authenticated/w/$wsId/outreach/lists/new/csv",
)({
  component: NewCsvList,
});

// stage не маппим: он определяется системой при конверсии лида → контакт
// (preset-default = первая опция воронки). Юзер не должен пытаться загрузить
// stage из CSV (потребовало бы валидации option_id).
const SKIP_PROPS_IN_MAPPING = new Set(["stage"]);

// Synonyms для авто-маппинга по заголовкам CSV. EXACT-match на lowercased
// header (после trim) — чтобы «telegram_username» не подхватился под `tel` для
// `phone`. Property.key и property.name автоматически добавляются в кандидаты.
...
function NewCsvList() {
  ...
  useEffect(() => {
    ...
    // Iterate properties в их order — если имя CSV-колонки даёт exact-match
    // на synonym/key/name property, занимаем колонку. Раньше идущие propertyies
    // имеют приоритет (telegram_username перед phone — чтобы не словить «tel»).
  ...
  // Backend expects propertyMappings: {propertyKey: csvColumn}. Конвертим из
  // нашей inverted-стороны (csvColumn → propertyKey).
  ...
  return (
    <div className="space-y-3 p-6">
      ...
      <div className="mx-auto max-w-2xl space-y-4">
        <h1 className="text-xl font-semibold">Загрузить CSV</h1>
        ...
        {parsed && (
          <>
            <div className="rounded-2xl bg-white p-5 shadow-sm">
              <Field label="Название списка">
                <input
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Лиды с конференции"
                  ...
                  >
                    ...
                    {mappableProps.map((p) => (
                      <option
                        ...
                        value={p.key}
                        disabled={
                          // Один property можно мапить только из одной колонки.
