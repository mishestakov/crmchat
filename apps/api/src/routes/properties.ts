...
import { channels, properties } from "../db/schema.ts";
import type { WorkspaceVars } from "../middleware/assert-member.ts";

// Каталог `properties` относится к КАНАЛАМ (channels.properties). Контакты
// каталога не имеют — у них фиксированный CONTACT_FIELD_DEFS (@repo/core).

// `type` в UpdatePropertySchema намеренно отсутствует: смена типа = миграция данных
// (пересборка channels.properties[key] под другой формат), это отдельный сценарий.

// TODO: при изменении property.values (особенно single_select) проверять, что
// убираемые option.id не используются ни в одном channels.properties[key].
...
app.openapi(
  ...
  async (c) => {
    ...
    // транзакции чистим их из channels.properties[key]:
    ...
    const row = await db.transaction(async (tx) => {
      ...
      if (removedIds.length > 0) {
        if (existing.type === "single_select") {
          // Required single_select → cleanup НЕ может оставить канал без значения
...
app.openapi(
  ...
  async (c) => {
    const wsId = c.get("workspaceId");
    const { id } = c.req.valid("param");
    // Транзакция: удалить property + почистить значение по этому key из всех
    // channels workspace'а. Иначе в JSONB остаются висячие ключи, которые позже
    // всплывут в экспорте/аналитике.
    await db.transaction(async (tx) => {
      ...
      await tx
        .update(channels)
        .set({ properties: sql`${channels.properties} - ${existing.key}` })
