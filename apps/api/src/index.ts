...
import { startBotClient } from "./lib/tg-bot.ts";
...
if (process.env.NODE_ENV !== "test") {
  ...
  // Бот-логин через TDLib (MTProto + прокси) — на RU-сервере надёжнее Bot API.
  void startBotClient().catch((e: unknown) =>
    console.error("[tg-bot] startBotClient failed:", e),
  );
}
