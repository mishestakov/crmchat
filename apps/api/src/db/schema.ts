...
export const legalEntities = pgTable(
  "legal_entities",
  {
    ...
    // возникает: агентство ≠ клиент ≠ блогер).
    //
    // АРХИТЕКТУРА ОРД (Фаза 2): эта таблица — КАНОНИЧЕСКОЕ хранилище (модель
    // ЕРИР, вендор-НЕЙТРАЛЬНО). Вендор-специфика — только в адаптере (маппинг в
    // wire-формат VK/Яндекс + вызовы API), НЕ в схеме. Статус синка — общая
    // таблица на ВСЕ ОРД-объекты и ПО-ОПЕРАТОРНО:
...
export type ProjectOpener = {
  ...
  // Проверочный вариант для сегмента «нет РКН» (>10к и не в реестре — красная
  // пилюля). Задан и непуст → эти каналы перестают быть терминалом blocked_rkn:
  // им шлётся rknText («а вы точно не в ЕРИР?») вместо отбраковки. Пусто/null →
  // сегмент остаётся отбракованным (поведение по умолчанию). Живёт в этом же
  // jsonb — новой колонки/миграции не нужно.
  rknText?: string | null;
};
...
export const projects = pgTable(
  "projects",
  {
    ...
    // Реквизиты рекламодателя переехали в legal_entities (юрлицо клиента-трека) —
    // единый источник для ЕРИД. Кампанийного free-text advertiser_data больше нет.
