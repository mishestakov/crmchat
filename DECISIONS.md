...
## MAX-интеграция: только BD-минимум, агентские куски выпилены

**Решение.** MAX (мессенджер) живёт только в BD-сценарии: подключение аккаунта, карточка канала (подписчики/охват/лента, вступление в закрытые), привязка админа по ссылке `max.ru/u/…` в карточке канала, ручная ЛС-переписка + realtime-входящие. Выпилены: метрики вышедших постов (`runMaxMetrics`/`MSG_GET_STAT`), MAX-ветка в агентском placement-drawer, бэкфилл имени контакта (`ensureMaxContactDisplay`), мёртвые методы клиента (`chatSubscribe`, `contactByPhone`). Полная версия — в теге `max-full`.

**В оригинале.** Ветка `max` писалась «всё сразу», включая трекинг просмотров вышедших размещений — это агентский сценарий (отчёт по кампании), а MAX в агентстве не используется.

**Почему.** 80/20: BD-цепочка «завёл канал → оценил → написал админу → получил ответ» не теряет ничего, минус ~300 строк кода с маленьким инкрементом ценности. Понадобятся метрики — `git show max-full` вернёт готовый код.

---

## Контакты: карточка без списка

**Решение.** Страница-список контактов (`/w/$wsId/contacts`) удалена. Остаётся только карточка контакта (`/w/$wsId/contacts/$id`) — на неё deep-link из канала, канбана, лидов. Посадочная после входа / создания воркспейса / приёма инвайта ведёт на «Площадки» (`/channels`).

**В оригинале.** Контакты были главной страницей CRM (плоский список всех людей).

**Почему.** Модель канало-центричная (этап 16+): людей находят через каналы, а не листают подряд. Пункт «Контакты» из меню убрали ещё на этапе витрины «Площадок», но посадочные редиректы остались висеть на удалённый список — входящий пользователь попадал на страницу, которой нет в навигации. Список как сущность дублировал поиск по каналам, продуктового смысла не нёс.
...
## Ручная пиналка (этап C): TOCTOU при взводе — гасим UI, не сервером

**Решение.** Защита от двойного взвода серии (два параллельных POST
`/items/{id}/dunning {enabled:true}`) — на фронте: кнопка `disabled` пока
мутация `pending`. Серверного барьера (transaction + `SELECT … FOR UPDATE` или
unique `(item_id, message_idx, dunning_round)`) в MVP нет.

**Почему.** `armLeadDunning` читает scheduled (проверка «уже идёт») и вставляет
новый заход не атомарно — теоретически дабл-клик создаёт две pending-серии и
ломает инвариант «одна серия за раз». Практически: единственный путь — два
клика быстрее одного ререндера; UI-`disabled` это закрывает. Серверный гард =
либо ручная аддитивная миграция на проде (unique-constraint), либо транзакция с
блокировкой ради edge-кейса дабл-клика — оверкилл для текущей фазы.

**Pre-prod TODO.** Если появится программный/ретраи-доступ к ручке (не только
кнопка) — добавить unique `(item_id, message_idx, dunning_round)` + upsert
on-conflict-do-nothing, тогда вторая серия отвалится на уровне БД.
