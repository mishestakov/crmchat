...
**Решение.** Останавливаем копирование donor'а. Меняем доменную модель под
рекламное агентство (Клиент → Проект → Размещение поверх общей базы Блогеров и
Каналов; финансы и артефакты first-class). Подробный план и фазы — в
[`specs/agency-pivot.md`](specs/agency-pivot.md).

**Что было.** До этого момента шёл feature-parity с donor (CRMChat for Web3-BD):
один глобальный канбан контактов, кампании как источники трафика, плоская
properties-модель.

**Почему.** Web3-сценарий и агентский — это **разные продукты с разной осью**:
- Web3: ось — персона, состояние = pipeline сделки с человеком.
- Агентство: ось — размещение (канал × проект × дата); канал переиспользуется
  между проектами, у проекта свой жизненный цикл и свой канбан.

Сложить два сценария в один продукт без архитектурного компромисса нельзя:
плоская модель `contact + properties` не выдержит структурированных запросов
типа «каналы тематика=финтех, гео=РФ, бюджет ≤ 50k за пост, ER ≥ 3%».

**План миграции.** Фазы P0 (заморозка legacy) → P9 (депрекация). См. agency-pivot.md.
Legacy остаётся работать в течение P1-P8, удаляется в P9.
